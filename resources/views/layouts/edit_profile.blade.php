			...
			<div class='row content'>
				<div style='margin-left:5%' class ='col-sm-5'>
					...
					<div class="form-group">
					    ...
					    <input type="phonenum" class="form-control" name="phonenum" id="phonenum" value="{{(Auth::user()->phonenum)}}" placeholder="enter contact number">
					    @if($errors->has('phonenum'))<div style='margin-top:5px' class="alert alert-danger"> <p>{{$errors->first('phonenum')}}</p> </div>@endif
					</div>	

				<div class="form-group">
				    <label for="company">Company name:</label>
				    <input type="company" class="form-control" name="company" id="company" value="{{(Auth::user()->company)}}" placeholder="optional">
				...
				<div style='margin-left:5%' class ='col-sm-5'>	
					...
					<div class="form-group">
					    ...
					    <input type="address" class="form-control" name="address" id="address" value="{{(Auth::user()->address)}}" placeholder="full address">
					    @if($errors->has('address'))<div style='margin-top:5px' class="alert alert-danger"> <p>{{$errors->first('address')}}</p> </div>@endif
					</div>

					<div class="form-group">
					    <label for="state">State:</label>
					    <input type="state" class="form-control" name="state" id="state" value="{{(Auth::user()->state)}}" placeholder="enter state">
					    @if($errors->has('state'))<div style='margin-top:5px' class="alert alert-danger"> <p>{{$errors->first('state')}}</p> </div>@endif
					</div>

					<div class="form-group">
					    <label for="country">Country:</label>
					    <input type="country" class="form-control" name="country" id="country" value="{{(Auth::user()->country)}}" placeholder="Country">
					    @if($errors->has('country'))<div style='margin-top:5px' class="alert alert-danger"> <p>{{$errors->first('country')}}</p> </div>@endif
					</div>					

					<div class="form-group">
					    <label for="zip">Zip code:</label>
					    <input type="zip" class="form-control" name="zip" id="zip" value="{{(Auth::user()->zip)}}" placeholder="zip code">
